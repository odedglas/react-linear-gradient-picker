...
const mapPaletteToStops = ({ palette, activeId, width }, setActiveColorId) => {
  ...
  const isHaveActive = result.find((color) => color.isActive === true);
  if (!isHaveActive) {
    result[0].isActive = true;
    setActiveColorId(result[0].id);
  }
  return result;
};
      ...
      >
        ...
        >
          <button
            ...
            style={{
              ...(palette.length <= minStops ? { cursor: "not-allowed" } : {}),
            }}
          >
